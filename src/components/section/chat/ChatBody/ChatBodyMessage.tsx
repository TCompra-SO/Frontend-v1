      ...
      >
        {props.message.images ? (
          props.message.images.map((img) => (
            <img src={img} alt="" className="image-send" key={img} />
          ))
        ) : props.message.documents ? (
          props.message.documents.map((doc) => (
            <div className="file-min-2" onClick={() => openDocument(doc)}>
        ...
        <span
          className={props.message.images ? "mensaje-hora-img" : "mensaje-hora"}
        >
